...
[![Circle CI](https://circleci.com/gh/owelinux/docker-nginx.svg?style=svg)](https://circleci.com/gh/owelinux/docker-nginx)

This is a [owelinux/nginx](https://registry.hub.docker.com/u/owelinux/nginx/) docker container with Nginx web server, nicely tuned for a better performance.
...
Folders `/application/nginx/` and `/data/conf/nginx/` are monitored for any config changes and, when they happen, Nginx is gracefully reloaded.
...
```
docker run -d --name=web -p=80:80 -p=443:443 -e "NGINX_GENERATE_DEFAULT_VHOST=true" owelinux/nginx
```

With data container:  
```
docker run -d --name=web-data -v /data busybox
docker run -d --name=web --volumes-from=web-data -p=80:80 -e "NGINX_GENERATE_DEFAULT_VHOST=true" owelinux/nginx
```
...
When set to `true`, dummy default (*catch-all*) Nginx vhost config file will be generated in `/application/nginx/hosts.d/default.conf`.  
